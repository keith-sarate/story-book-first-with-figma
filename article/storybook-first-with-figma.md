...
**The first thing the dev agent does is refine the story.** When `bmad-dev-story` activates on a PM-authored story, BMad's customization resolver reads `_bmad/custom/bmad-dev-story.toml` and loads every entry in its `persistent_facts` array as foundational context. One of those facts mandates a Step 0 *before* the per-component loop kicks in: resolve the Figma link from the story's References, classify the target (atom / molecule / organism / screen), and **decompose it recursively** to find every lower-layer component it depends on. The agent then runs a Reuse Check on every node of that dependency tree and produces a **bottom-up build plan** — atoms first, then their consumers, finally the target. The plan is written back to the story's Tasks before any code is written.

**This matters in brownfield adoption.** In real teams, the module gets installed into a project that has been running for years. The first story your dev agent picks up may target a full checkout organism or a settings screen — and there will be no atoms in `src/components/atoms/` yet. The agent does not HALT on missing dependencies; it decomposes the target, classifies each piece, and builds bottom-up in a single dev-story run. Each component still passes through the 5-step loop (read Figma → reuse check → build → validate → sync) and lands in the correct layer folder. The user sees the build plan before code is written; if more than three components will be created, the agent surfaces the plan for confirmation. The result: one PM-authored story can yield twenty atoms + five molecules + three organisms + the target screen, all properly placed, all visually validated, all synced back to a `Built / <Layer>` page in Figma — without anyone having to author twenty extra stories upfront.

The two cases where the agent does HALT during Step 0:
- **UPDATE conflict** — a component already exists at the right layer but disagrees with the Figma source. The agent surfaces the conflict and asks whether to update the existing component, keep it, or fork.
- **Off-layer brownfield component** — a component exists in the codebase but outside the atomic structure (e.g., flat `src/components/Button.tsx`). The agent surfaces the discovery and asks whether to refactor it into `atoms/`, reuse from current location, or build a parallel atomic version. This is a design-system decision the agent will not make alone.
